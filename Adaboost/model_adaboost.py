...
OUT_PATH = os.path.join(folder, "adaboost_submission.csv")
...
base = DecisionTreeRegressor(max_depth=10, min_samples_leaf=5, random_state=SEED)
model = AdaBoostRegressor(estimator=base, n_estimators=250, learning_rate=0.07, random_state=SEED)
model.fit(X_scaled, y)
preds_log = model.predict(X_test_scaled)
preds = np.expm1(preds_log)
pd.DataFrame({'Id': test_ids, 'HotelValue': preds}).to_csv(OUT_PATH, index=False)
print(f"AdaBoost submission written to {OUT_PATH}")
