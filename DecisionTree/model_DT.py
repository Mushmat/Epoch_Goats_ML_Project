...
OUT_PATH = os.path.join(folder, "dt_submission.csv")
...
model = DecisionTreeRegressor(max_depth=14, min_samples_split=8, min_samples_leaf=3, random_state=SEED)
model.fit(X_scaled, y)
preds_log = model.predict(X_test_scaled)
preds = np.expm1(preds_log)
pd.DataFrame({'Id': test_ids, 'HotelValue': preds}).to_csv(OUT_PATH, index=False)
print(f"Decision Tree submission written to {OUT_PATH}")
