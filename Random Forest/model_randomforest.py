...
OUT_PATH = os.path.join(folder, "rf_submission.csv")
...
model = RandomForestRegressor(n_estimators=300, max_depth=18, min_samples_split=6, random_state=SEED, n_jobs=-1)
model.fit(X_scaled, y)
preds_log = model.predict(X_test_scaled)
preds = np.expm1(preds_log)
pd.DataFrame({'Id': test_ids, 'HotelValue': preds}).to_csv(OUT_PATH, index=False)
print(f"Random Forest submission written to {OUT_PATH}")
