...
def advanced_feature_engineering(df):
    ...
    df['OverallScore'] = (df['OverallQuality'] + df['OverallCondition']) / 2
...
# 3. OUTLIER REMOVAL (adjusted to 2-98 percentile)
def remove_outliers_advanced(df, target_col='HotelValue'):
    before = len(df)
    Q1 = df[target_col].quantile(0.02)
    Q3 = df[target_col].quantile(0.98)
    IQR = Q3 - Q1
    lower = Q1 - 1.5 * IQR
    upper = Q3 + 1.5 * IQR
...
ridge.fit(X_tr, y_tr)
models['Ridge'] = {'model': ridge, 'rmse': np.sqrt(mean_squared_error(y_val, ridge.predict(X_val)))}

lasso = Lasso(alpha=0.0005, max_iter=10000, random_state=SEED)
lasso.fit(X_tr, y_tr)
models['Lasso'] = {'model': lasso, 'rmse': np.sqrt(mean_squared_error(y_val, lasso.predict(X_val)))}

elastic = ElasticNet(alpha=0.0005, l1_ratio=0.5, max_iter=10000, random_state=SEED)
elastic.fit(X_tr, y_tr)
models['ElasticNet'] = {'model': elastic, 'rmse': np.sqrt(mean_squared_error(y_val, elastic.predict(X_val)))}

bayesian = BayesianRidge(compute_score=True)
bayesian.fit(X_tr, y_tr)
models['BayesianRidge'] = {'model': bayesian, 'rmse': np.sqrt(mean_squared_error(y_val, bayesian.predict(X_val)))}
...
models['XGBoost'] = {'model': xgb_model, 'rmse': np.sqrt(mean_squared_error(y_val, xgb_model.predict(X_val)))}

# Optimized GradientBoosting: lower learning rate, more estimators
gb_model = GradientBoostingRegressor(
    n_estimators=700, max_depth=4, learning_rate=0.04, subsample=0.8,
    min_samples_split=5, min_samples_leaf=2, random_state=SEED
)
gb_model.fit(X_tr, y_tr)
models['GradientBoosting'] = {'model': gb_model, 'rmse': np.sqrt(mean_squared_error(y_val, gb_model.predict(X_val)))}

print("\nModel Performance:")
for name, info in sorted(models.items(), key=lambda x: x[1]['rmse']):
    print(f"{name:20s} RMSE: {info['rmse']:.6f}")

# 6. ENSEMBLE - Optimized weights favoring GradientBoosting
...
for name, info in models.items():
    ensemble_preds_log += info['model'].predict(X_test_scaled) * weights[name]

final_preds = np.expm1(ensemble_preds_log)
# No clipping - let the ensemble decide
...
print(f"  - Weights: GBM 32%, XGB 20%, Ridge/Bayesian 14%, Lasso 11%, ElasticNet 9%")
print(f"  - Outlier removal: 2–98% quantile")
print(f"  - No prediction clipping")
print("="*80)
